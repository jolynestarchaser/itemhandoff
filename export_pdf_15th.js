...
async function main() {
  ...
  // Filter departments that have ONLY 1 date, and that date is 2026-07-15
  const targetDateStr = '2026-07-15';
  const qualifyingDepts = [];

  for (const dept of Object.keys(deptMap)) {
    const recs = deptMap[dept];
    const uniqueDates = new Set(recs.map(r => formatDateStr(r.handoffDate || r.createdAt)));
    const dateArray = Array.from(uniqueDates);
    
    if (dateArray.length === 1 && dateArray[0] === targetDateStr) {
  ...
  console.log(`\nFound ${qualifyingDepts.length} departments that ONLY have records on ${targetDateStr}:`);
    ...
    <div class="document-style">
      ...
      <div class="header-info">
        <div class="info-row">
          ...
          <div class="info-dots">${formattedDate}</div>
        </div>
        <div class="info-row">
          <div class="info-label">แผนก</div>
          <div class="info-dots">${deptNameTh}</div>
        </div>
      </div>
  ...
  <head>
    <meta charset="UTF-8">
    <title>ใบส่งสินค้าชั่วคราว - ประจำวันที่ 15 กรกฎาคม 2569</title>
  ...
  const outputHtmlPath = path.join(__dirname, 'delivery_notes_15th_single_date.html');
  const outputPdfPath = path.join(__dirname, 'delivery_notes_15th_single_date.pdf');
